...
describe('TESTING A ROUTER DE SESSIONS', function() {
    ...
    it('Prueba endpoint POST /login => Registra un usuario en BD, en caso de exito rerige a /current. Adenas genera un token de seguridad que se guarda en cookies.', async function(){
        let datos = {email: user.email, password: user.password}
        let respuesta = await requester.post('/api/sessions/login').send({ ...datos })
...
/* 
    it('Prueba Endpoint GET /api/sessions/github => /callbackGithub el endpoint utiliza una estrategia personalizada, que ingresa con credenciales privadas del perfil de github y luego obtiene los datos del usuario', async function(){
        let respuesta = await requester.get('/api/sessions/github')
        console.log(respuesta)

        expect(respuesta).exist
    })
 */
